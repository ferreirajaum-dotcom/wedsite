...
const PricingCard: React.FC<PricingCardProps> = ({ 
  ...
  const theme = {
    ...
    orange: {
      containerClasses: 'border-amber-500/50 bg-[#151E32] shadow-[0_0_30px_rgba(245,158,11,0.1)] hover:border-amber-400 hover:shadow-[0_0_50px_rgba(245,158,11,0.3)] hover:-translate-y-2',
      tag: 'bg-amber-500 text-black shadow-lg shadow-amber-500/40',
      button: 'bg-gradient-to-r from-amber-500 to-orange-600 text-white shadow-lg shadow-orange-500/25 hover:shadow-orange-500/50 hover:scale-[1.02] active:scale-[0.98] hover:brightness-110',
  ...
  return (
    <ScrollReveal delay={delay} className="h-full">
      <div className={`relative flex flex-col p-6 rounded-2xl border transition-all duration-500 ease-out-expo h-full group ${styles.containerClasses}`}>
        
        {/* Tag Superior com animação suave */}
        {tagText && (
          <div className={`absolute -top-4 left-1/2 -translate-x-1/2 px-4 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-wider flex items-center gap-1 transition-transform group-hover:scale-105 ${styles.tag}`}>
        ...
        {/* Fidelity Section */}
        <div className="mb-6">
          <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-2">Fidelidade do Plano</p>
          ...
          <ul className="space-y-3">
            {features.map((feature, idx) => (
              <li key={idx} className="flex items-start gap-3 text-sm text-slate-300 group-hover:text-slate-200 transition-colors">
                <Check className={`w-4 h-4 mt-0.5 flex-shrink-0 transition-transform group-hover:scale-110 ${styles.check}`} />
        ...
        {/* CTA Button */}
        <button className={`w-full py-3.5 rounded-lg font-bold text-sm transition-all duration-300 ${styles.button}`}>
          {ctaText}
        </button>
      </div>
    </ScrollReveal>
